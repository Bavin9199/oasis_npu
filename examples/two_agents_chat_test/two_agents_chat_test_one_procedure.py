...
def critique_ai_style(draft_reply, agent_desc):
    """
    ...
    critique_prompt = f"""
        ...
        【修改建议】
        - 用要点列表提出针对性的修改建议
        - 根据每一条拟真维度分别提出进一步修改的建议，尤其是对字数进行严格限制
...
def rewrite_with_critic(id, draft_reply, critique):
    ...
    final_prompt = [
    {
        ...
        你必须严格执行修改要求中的字数限制，输出必须严格控制，若无法同时表达所有情绪，请优先保留最重要的信息，不必完整呈现所有细节。
        你的任务是在不脱离该虚拟用户人设的前提下；适当保留合适的标点符号使用特征；将接下来提供的草稿内容重写得更生活化、更口语化、更自然、更像真人。
